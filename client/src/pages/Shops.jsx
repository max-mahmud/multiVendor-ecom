...
const Shops = () => {
    ...
    const { products, totalProduct, categorys, latest_product, priceRange } = useSelector(state => state.home)
    useEffect(() => {
                    ...
                    <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                        <div className='flex flex-col justify-center gap-1 items-center h-full w-full text-white'>
                            ...
                            <div className='flex justify-center items-center gap-2 text-2xl w-full'>
                                <Link to='/'>Home</Link>
                                <span className='pt-1'><MdOutlineKeyboardDoubleArrowRight /></span>
                                <span>Products</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className={`w-3/12 md-lg:w-4/12 md:w-full pr-8 ${filter ? 'md:h-0 md:overflow-hidden md:mb-6' : 'md:h-auto md:overflow-auto md:mb-0'}`}>
                            ...
                            <div className='py-2'>
                                {
                                    categorys.map((c, i) => <div className='flex justify-start items-center gap-2 py-1' key={i}>
                                        <input checked={category === c.name ? true : false} onChange={(e) => queryQategory(e, c.name)} type="checkbox" id={c.name} className='cursor-pointer' />
                            ...
                            <div className='py-3 flex flex-col gap-4'>
                                ...
                                <div className='flex flex-col gap-3'>
                                    ...
                                    <div onClick={() => setRatingQ()} className='text-orange-500 flex justify-start items-start gap-2 text-xl cursor-pointer'>
                                        <span><CiStar /></span>
                                        <span><CiStar /></span>
                                        <span><CiStar /></span>
                                        <span><CiStar /></span>
                            ...
                            <div className='pl-8 md:pl-0'>
                                <div className='py-4  mb-10 px-3 rounded-md flex justify-between items-start border bg-slate-50'>
                                    ...
                                    <div className='flex justify-center items-center gap-3'>
                                        <select onChange={(e) => setSortPrice(e.target.value)} className='p-1 border outline-0 text-slate-600 font-semibold bg-slate-100' name="" id="">
                                            <option value="">Sort By</option>
                                            <option value="low-to-high">Low to High Price</option>
                                            <option value="high-to-low">High to Low Price</option>
                                        </select>
                                        <div className='flex justify-center items-start gap-4 md-lg:hidden'>
                                            <div onClick={() => setStyles('grid')} className={`p-2 ${styles === 'grid' ? 'bg-slate-300' : 'bg-slate-100'}  text-slate-600 hover:bg-slate-300 cursor-pointer rounded-sm`}>
                                                <IoGrid />
                                            </div>
                                            <div onClick={() => setStyles('list')} className={`p-2 ${styles === 'list' ? 'bg-slate-300' : 'bg-slate-100'}  text-slate-600 hover:bg-slate-300 cursor-pointer rounded-sm`}>
                                                <FaThList />
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <Pagination pageNumber={pageNumber} setPageNumber={setPageNumber} totalItem={20} perPage={perPage} showItem={6} />
                                </div>
                            </div>
                        </div>
                    </div>
